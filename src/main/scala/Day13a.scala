...
object Day13a {
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    val allPoints = walk( Point(1,1), Point(31,39), grid )
    
    Console.println( allPoints.size )
  ...
  def walk( start : Point, target : Point, grid : Array[Array[Char]] ) : List[Point] = {
    ...
    while( !work.isEmpty ){
      ...
      if( current.points.length == 51 ){
        // done walking
        Console.println("done walking")
