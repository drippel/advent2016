...
object Day11 {
  ...
  def main( args : Array[String] ) : Unit = {
    ...
    /*
    Console.println( isFloorSafe( List( 1, 1, 1, 1, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 1, 1, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 0, 1, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 0, 0, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 1, 0, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 1, 1, 1 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 1, 1, 0 ) ) )
    Console.println( isFloorSafe( List( 1, 0, 0, 0, 0 ) ) )
    */
  ...
  def move( start : Building, from : Int, to : Int, mv : List[Int] ) : Option[Building] = {
    ...
    if ( start.steps.exists( equals( _, next ) ) ) {
      Console.println( "no back track..." )
      None
    } 
    else if( next.steps.size > shortest ){
      Console.println( "already has shorter..." )
      None
    }
    else {
      Some( next )
